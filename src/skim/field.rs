...
lazy_static! {
    static ref FIELD_RANGE: Regex =
        Regex::new(r"^(?P<left>-?\d+)?(?P<sep>\.\.)?(?P<right>-?\d+)?$").unwrap();
}
    ...
    fn test_parse_transform_fields() {
        ...
        assert_eq!(
            super::parse_transform_fields(
                &re,
                "A,B,C,D,E,F",
                &[Single(2), Single(4), Single(-1), Single(-7)]
            ),
            "B,D,F"
        );
