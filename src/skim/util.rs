...
use crate::skim::field::get_string_by_range;
use crate::skim::AnsiString;
    ...
    fn print_ch_to_canvas(&mut self, canvas: &mut dyn Canvas, ch: char, attr: Attr, skip: bool) {
        ...
        if !skip {
            let _ = canvas.put_cell(
                self.row,
                self.screen_col,
                Cell::default().ch(ch).attribute(attr),
            );
        }
...
pub fn margin_string_to_size(margin: &str) -> Size {
    if margin.ends_with('%') {
        Size::Percent(min(
            100,
            margin[0..margin.len() - 1].parse::<usize>().unwrap_or(100),
        ))
    } else {
...
pub fn inject_command<'a>(cmd: &'a str, context: InjectContext<'a>) -> Cow<'a, str> {
    RE_FIELDS.replace_all(cmd, |caps: &Captures| {
        ...
        let replacement = match range {
            ...
            _ => get_string_by_range(context.delimiter, context.current_selection, range)
                .unwrap_or(""),
        };
...
pub fn atoi<T: FromStr>(string: &str) -> Option<T> {
    RE_NUMBER
        .find(string)
        .and_then(|mat| mat.as_str().parse::<T>().ok())
}
