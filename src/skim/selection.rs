...
use crate::skim::item::MatchedItem;
use crate::skim::orderedvec::OrderedVec;
use crate::skim::theme::{ColorTheme, DEFAULT_THEME};
use crate::skim::util::clear_canvas;
use crate::skim::util::{print_item, reshape_string, LinePrinter};
use crate::skim::{DisplayContext, MatchRange, Matches, Selector, SkimItem, SkimOptions};
use regex::Regex;
use std::rc::Rc;
            ...
            let (match_start_char, match_end_char) = match matched_item.matched_range {
                Some(MatchRange::Chars(ref matched_indices)) => {
                    if !matched_indices.is_empty() {
                        (
                            matched_indices[0],
                            matched_indices[matched_indices.len() - 1] + 1,
                        )
                    } else {
                        (0, 0)
                    }
    ...
    fn draw(&self, canvas: &mut dyn Canvas) -> DrawResult<()> {
        ...
        for item_idx in item_idx_lower..item_idx_upper {
            ...
            let _next_col = canvas
                .print_with_attr(line_no, 0, label, self.theme.cursor())
                .unwrap();
...
impl Widget<Event> for Selection {
    fn on_event(&self, event: TermEvent, _rect: Rectangle) -> Vec<Event> {
        ...
        match event {
            ...
            TermEvent::Key(Key::DoubleClick(MouseButton::Left, ..)) => {
