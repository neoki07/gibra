...
use crate::skim::{ItemPreview, PreviewContext, PreviewPosition, SkimItem};
...
impl Previewer {
        ...
        let thread_previewer = thread::spawn(move || {
            run(rx_preview, move |lines, pos| {
                ...
                hscroll_offset_clone
                    .store(max(1, max(hscroll, hoffset) - hoffset), Ordering::SeqCst);
                vscroll_offset_clone
                    .store(max(1, max(vscroll, voffset) - voffset), Ordering::SeqCst);
    ...
    fn act_scroll_down(&mut self, diff: i32) {
        ...
        self.vscroll_offset
            .store(max(new_offset, 1), Ordering::SeqCst);
    }
    ...
    fn act_scroll_right(&mut self, diff: i32) {
        ...
        self.hscroll_offset
            .store(max(1, new_offset), Ordering::SeqCst);
    }
...
impl Widget<Event> for Previewer {
    fn on_event(&self, event: TermEvent, _rect: Rectangle) -> Vec<Event> {
        let mut ret = vec![];
        match event {
            TermEvent::Key(Key::WheelUp(.., count)) => {
                ret.push(Event::EvActPreviewUp(count as i32))
            }
            TermEvent::Key(Key::WheelDown(.., count)) => {
                ret.push(Event::EvActPreviewDown(count as i32))
            }
            _ => {}
        }
        ret
                ...
                match spawned {
                    Err(err) => {
                        let astdout = AnsiString::parse(
                            format!("Failed to spawn: {} / {}", cmd, err).as_str(),
                        );
    ...
    fn adjust_scroll_print(&self, canvas: &mut dyn Canvas, ch: char, attr: Attr) -> Result<usize> {
        if self.row < self.skip_rows || self.col < self.skip_cols {
            ...
            canvas.put_char_with_attr(
                self.row - self.skip_rows,
                self.col - self.skip_cols,
                ch,
                attr,
            )
        }
