...
use crate::skim::event::{parse_event, Event};
use regex::Regex;
...
mod test {
    ...
    fn execute_should_be_parsed_correctly() {
        ...
        assert_eq!(
            ("ctrl-m", vec![("execute", Some(cmd.to_string()))]),
            key_action[1]
        );
        ...
        let key_action = parse_key_action(key_action_str);
        assert_eq!(
            ("f1", vec![("execute", Some("less -f {}".to_string()))]),
            key_action[0]
        );
        assert_eq!(
            (
                "ctrl-y",
                vec![("execute-silent", Some("echo {} | pbcopy".to_string()))]
            ),
            key_action[1]
        );
        ...
        assert_eq!(
            (
                "enter",
                vec![("execute", Some("$EDITOR +{2} {1}".to_string()))]
            ),
            key_action[0]
        );
    }
    ...
    fn action_chain_should_be_parsed() {
        ...
        assert_eq!(
            ("ctrl-t", vec![("toggle", None), ("up", None)]),
            key_action[0]
        );
